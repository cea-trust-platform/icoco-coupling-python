...
```math
\rho_i \dfrac{\partial H_i}{\partial t} + G_i \dfrac{\partial H_i}{\partial z} = h_i (T_s - T_fi)
```

For the plate, we have a kind of 0D model:

```math
C \dfrac{\partial T_s}{\partial t} = h_1 (T_s - T_f1) + h_2 (T_s - T_f2) + P
```
...
* T_fi: fluid temperature in channel i.
...
H_i is provided as a function of T_fi. h_1 and h_2 are provided as functions of T_s.
...
```math
H_i^{j} = H_i^{j-1} + dz / G_i * h_i^{j} (T_s^{j} - T_fi^{j})
```

Here dz is the cell size (taken constant) and the superscript $j$ stands for the cell index.

The function providing H_i as a function of T_fi is then inverted using a Newton algorithm to compute a new guess for T_fi^{j}.
...
```math
\rho_i \dfrac{H_i^{j, n+1} - H_i^{j, n}}{dt} + G_i \dfrac{H_i^{j, n} - H_i^{j-1, n}}{dz} = h_i^{j, n+1} (T_s^{j, n+1} - T_fi^{j, n+1})
```
